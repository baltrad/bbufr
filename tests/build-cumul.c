...

{
  ...
  varfl    periode = MISSVAL, calib_method = MISSVAL;
  varfl    accu_method = MISSVAL, fact_corr = MISSVAL, time_sig = MISSVAL;
  ...
  while (i < ndds) {
    ...
    if (test_fxy(d, 3, 1, 1)) {
      ...
      bufr_parse (d, 0, 0, vals, &vali, value_out);
      i++;
      if (i == 2) time_sig = global_val[0]; 
      mindex = 0;
    } else if (test_fxy(d, 0, 4, 25)) {
      static int i = 0;
      bufr_parse (d, 0, 0, vals, &vali, value_out);
      i++;
      if (i == 2) periode = global_val[0]; 
      mindex = 0;
      ...
      mindex = 0;
    } else if (test_fxy(d, 3, 21, 7)) {
      bufr_parse (d, 0, 0, vals, &vali, value_out);
      calib_method = global_val[0]; /* 0 25 9 */
      mindex = 0;
    } else if (test_fxy(d, 0, 25, 196)) {
      bufr_parse (d, 0, 0, vals, &vali, value_out);
      accu_method = global_val[0];
      mindex = 0;
    } else if (test_fxy(d, 0, 25, 210)) {
      bufr_parse (d, 0, 0, vals, &vali, value_out);
      fact_corr = global_val[0];
      mindex = 0;
	...
	pixmap = calloc(ncols*nrows, sizeof(unsigned int));
...
/******* If there is a replication factor -> increase i by the number of 
...

  {
    ...
    sprintf(str, "+proj=aeqd +lat_0=%g +lon_0=%g +a=%g +es=0", 
	    lat_rad, lon_rad, rterre);
    if (!(pj = pj_init_plus(str))) {
      fprintf(stderr, "FATAL: error init proj %s\n", pj_strerrno(pj_errno));
      return 0;
    }
    p.v = lat_rad*DEG_TO_RAD;
    p.u = lon_rad*DEG_TO_RAD;
    p = pj_fwd(p,pj);
    ...
    fill_v(rterre);
    fill_v(rterre);
    fill_v(lon_rad); /* longitude origin */
    fill_v(lat_rad); /* latitude origin */
    fill_v(p.u); 
    fill_v(p.v); 
    ...
    fill_v(2);
    if (picture_type != 14) {
      fprintf (stderr, "WARNING: not correct picture type %g\n", picture_type);
    }
  ...
  fill_desc(0,25,9);
  fill_v(2); /* bit 3 à 1 pour 4 bits de taille ==> valeur 2 */
  if (calib_method != 2) { 
    fprintf (stderr, "WARNING: not correct calibration method %g\n", calib_method);
  }

  fill_desc(0,25,192);
  fill_v(accu_method);
  if (accu_method != 1) {
    fprintf (stderr, "WARNING: not correct accumulation method\n");
  }
  ...
  if (pixmap) {
    int jj;
    unsigned int *p = calloc(ncols, sizeof(unsigned int));
    varfl *ptr = NULL;
    int count = 0;
    for (ii = 0; ii < nrows; ii++) {
      for (jj = 0; jj < ncols; jj++) {
	p[jj] = pixmap[ii*ncols + jj];
      ...
      if (!rlenc_compress_line_new (ii, p, ncols, &ptr, &count)) {
