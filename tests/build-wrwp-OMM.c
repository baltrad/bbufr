...
/* Create a new OPERA BUFR message */
{
  ...
  s1.subcent = s1_in->subcent;
  s1.gencent = s1_in->gencent;
  ...
  s1.vmtab = 6;                     /* version number of master table used */
  s1.vltab = 0;                     /* version number of local table used */
...
{
  dd *dds, *d;
  int ndds, i, ii, iout, v_size;
  unsigned int vali, jout;
  size_t nvals;
  varfl *vals;
  char *unit;

  varfl    region = MISSVAL, station = MISSVAL, sign = MISSVAL, displa = MISSVAL;
  varfl    lat_rad = MISSVAL, lon_rad = MISSVAL, hauteur = MISSVAL, type = MISSVAL;
  ...
  varfl    gate = MISSVAL, speed = MISSVAL, wind = MISSVAL, capteur = MISSVAL;
  varfl    antenne = MISSVAL, beam = MISSVAL, frequence = MISSVAL;
  ...
  while (i < ndds) {
    d = &dds[i];
    unit = get_unit (d);
    if (test_fxy(d, 3, 1, 32)) {
      bufr_parse (d, 0, 0, vals, &vali, value_out);
      region = global_val[0];
      station = global_val[1];
      type = global_val[2];
      an_3_1_11 = global_val[3];
      mois_3_1_11 = global_val[4];
      jour_3_1_11 = global_val[5];
      heure_3_1_12 = global_val[6];
      min_3_1_12 = global_val[7];
      lat_rad = global_val[8];
      lon_rad = global_val[9];
      hauteur = global_val[10]; 
      mindex = 0;
    } else if (test_fxy(d, 0, 2, 3)) {
      bufr_parse (d, 0, 0, vals, &vali, value_out);
      capteur = global_val[0]; 
      mindex = 0;
    } else if (test_fxy(d, 0, 2, 101)) {
      bufr_parse (d, 0, 0, vals, &vali, value_out);
      antenne = global_val[0]; 
      mindex = 0;
    } else if (test_fxy(d, 0, 2, 106)) {
      bufr_parse (d, 0, 0, vals, &vali, value_out);
      beam = global_val[0]; 
      mindex = 0;
    } else if (test_fxy(d, 0, 2, 121)) {
      bufr_parse (d, 0, 0, vals, &vali, value_out);
      frequence = global_val[0]; 
      mindex = 0;
    } else if (test_fxy(d, 0, 25, 1)) {
      bufr_parse (d, 0, 0, vals, &vali, value_out);
      gate = global_val[0]; 
      mindex = 0;
    } else if (test_fxy(d, 0, 25, 20)) {
      bufr_parse (d, 0, 0, vals, &vali, value_out);
      speed = global_val[0]; 
      mindex = 0;
    } else if (test_fxy(d, 0, 25, 21)) {
      bufr_parse (d, 0, 0, vals, &vali, value_out);
      wind = global_val[0];
      mindex = 0;
      ...
      displa = global_val[0];
      mindex = 0;
    } else if (test_fxy(d, 1, 11, 0)) {
      bufr_parse (d, 0, 0, vals, &vali, value_out);
      nseuils = global_val[0];
      seuils = calloc(9*nseuils, sizeof(varfl));
      memcpy(seuils, global_val+1, 9*nseuils*sizeof(varfl));
      mindex = 0;
    } else {
...
/******* If there is a replication factor -> increase i by the number of 
  ...
  if (seuils) {
    ...
    for (ii = 0; ii < nseuils; ++ii) {
      fill_v(seuils[9*ii]);
      fill_v(seuils[9*ii+1]); /* 1 */
      fill_v(seuils[9*ii+2]); /* asso */
      fill_v(seuils[9*ii+3]);
      fill_v(seuils[9*ii+4]);
      fill_v(seuils[9*ii+5]); /* 1 */
      fill_v(seuils[9*ii+6]); /* asso */
      fill_v(seuils[9*ii+7]);
      fill_v(seuils[9*ii+8]);
    }
    free(seuils);
  }
